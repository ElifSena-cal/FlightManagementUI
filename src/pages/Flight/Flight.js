...
function User() {
    ...
    function handleDelete(id) {
        ...
        if (confirmDelete) {
            ...
            fetch(`http://localhost:8080/flights/delete/${userId}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('accessToken')}`, // Bearer tokeni ekleyin
                }
    ...
    function handleUpdate(id) {
        ...
        navigate(`/flight/Edit/${id}`);
    }
    return (
        <div className='container'>
            <h2 style={{ marginTop: '20px' }}>Flights</h2>
            <Link to="/flight/create" className='btn btn-success my-3'>Create +</Link>
            {loading ? (
                <p>Loading...</p>
            ) : (
                <div>
